...
module.exports = (data, html, purchaseOrder, deliveryNote) => {
  const form = new FormData();
  form.append('html', html);
  form.append('files[purchase-order.pdf]', purchaseOrder);
  if (deliveryNote) {
    form.append('files[delivery-note.pdf]', deliveryNote);
  }
  ...
  const params = {
    ...
    subject: `Purchase Order from ${data.order.billing_address.company} (${data.order.id})`,
